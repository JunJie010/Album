...
## 项目功能
- **双模式切换**  
  主界面提供两种工作模式：点击“开始相册”进入**相册浏览模式**，点击“开始幻灯片”进入**幻灯片自动播放模式**。

- **相册浏览**  
  在相册模式下，可通过触摸左侧区域（x < 90）**上一张**、右侧区域（x > 710）**下一张**，当前图片实时刷新显示；点击底部区域返回主界面。

- **幻灯片播放**  
  在幻灯片模式下，点击指定区域（250 < x < 550, y < 150）开始自动轮播所有图片，每张图片停留约 2 秒；点击底部区域可随时返回主界面。

- **触摸交互**  
  利用 Linux 输入子系统精确捕获触摸屏事件，将原始坐标映射到屏幕分辨率，实现基于坐标区域的交互逻辑，响应准确流畅。

- **图片渲染**  
  直接操作 FrameBuffer 进行 24 位 BMP 图片解码与显示，支持手动和自动翻页，画面清晰无卡顿。
